    ...
    def invoke() -> str:
        ...
        if result.returncode:
            raise PostReleaseError(
                result.stderr.strip() or result.stdout.strip() or f"gh {' '.join(args)} failed")
...
def _dispatch_and_wait(repo: str, workflow: str, ref: str, inputs: dict[str, str],
    ...
    while time.monotonic() < deadline:
        runs = json.loads(_gh(["api",
                              ...
                              "-f", "event=workflow_dispatch", "-f", "per_page=10",
                              "--jq", ".workflow_runs[] | {id, head_sha, created_at, status, conclusion}"]))
